...
def getPage(url):	
	...
	#import codecs
	#f = codecs.open('F:/1.txt','w','utf-8')
	#f.write(html)
	
	pattern=re.compile('<img src="(.{40,90}\.jpg)"|org_src="(.+?\.gif)',re.S)
	items=re.findall(pattern,html)
	for item in items:
		if not os.path.exists('F:/妹子图2'):
			os.mkdir('F:/妹子图2')
		global x
		try:
			urllib.request.urlretrieve('http:'+item[0],'F:/妹子图2/%s.jpg'%x)
			x+=1
		except:
			pass
		try:
			urllib.request.urlretrieve('http:'+item[1],'F:/妹子图2/%s.gif'%x)
			x+=1
		except:
			pass

for num in range(182,71,-1):
	print(num)
	url='http://www.jandan.net/ooxx/page-'+str(num)+'#comments'
	print(url)
	getPage(url)
